...
数据库使用 SQLite (libsql)，ORM 为 drizzle-orm。迁移在 Next.js 启动时自动执行（`src/instrumentation.ts` → `src/server/db/index.ts` 的 `migrate()`），用户升级代码重启服务即可平滑升级数据库。

修改 schema 后的流程：
1. 修改 `src/server/db/schema/*.ts`
2. 在终端交互式运行 `npx drizzle-kit generate`（需要 TTY 确认列变更）
3. 检查生成的 SQL，确保 `DROP TABLE` 使用 `IF EXISTS`
4. **禁止**手写自定义迁移再覆盖同样的变更，会导致重复执行报错
5. 迁移文件、snapshot 和 `_journal.json` 必须保持一致
