...
- `owner` — 站长，初始化时通过 `/setup` 创建，拥有所有权限。
- `admin` — 管理员，拥有所有权限，但不可添加、修改、删除 `owner`。
- `editor` — 编辑，可查看和编辑所有内容（文章、分类、标签、页面、评论、留言），但不可修改站点设置或管理其他用户。

注意事项：

- **系统不开放公开注册**，所有用户必须由 `owner` / `admin` 在后台创建。
- **editor 对所有内容有写权限**，即可以编辑他人创建的帖子和页面，但不可创建用户，也不可修改站点设置。
