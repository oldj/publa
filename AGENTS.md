...
数据库需要支持 SQLite 和 PostgreSQL，ORM 为 drizzle-orm。迁移在 Next.js 启动时自动执行（`src/instrumentation.ts` → `src/server/db/index.ts` 的 `migrate()`），用户升级代码重启服务即可平滑升级数据库。

修改 schema 后的流程：
1. 修改 `src/server/db/schema/define.ts`
2. 在终端交互式运行 `npm run db:generate:all`，生成所有迁移文件
