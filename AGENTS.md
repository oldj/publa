...
失败响应允许追加可选 `meta` 子对象，承载结构化的语义提示：

```ts
{ success: false, code: "RATE_LIMITED", message: "...", meta: { captchaShouldRefresh: true } }
```

- `meta` 仅在有实际语义需要传递时携带；不携带时响应体里不应出现该字段
- `meta` 的每个 key 必须有明确语义并在此处登记，对应类型在 `src/server/lib/api-response.ts` 的 `JsonErrorMeta` 接口中维护：
  - `captchaShouldRefresh: boolean` — 该次失败后前端是否应当立即刷新 captcha 输入；覆盖「缺 sessionId」「缺 captchaCode」「verifyCaptcha 已消费 captcha 行」三种场景
- 新增 meta key 时**同时**更新本文档与 `JsonErrorMeta` 接口，缺一不可
